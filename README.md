<a href="https://github.com/miguel50flowers">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12,2,20,30&height=180&section=header&text=Miguel%20Angel%20Esparza&fontSize=42&fontAlignY=38&fontColor=ffffff&desc=Full-Stack%20Engineer%20%C2%B7%20Shopify%20%C2%B7%20Quito%2C%20Ecuador&descAlignY=58&descSize=15&animation=fadeIn" alt="Miguel Angel Esparza — Full-Stack Engineer header" />
</a>

<p align="right">
  <img src="https://komarev.com/ghpvc/?username=miguel50flowers&style=flat-square&color=blueviolet&label=Profile+views" alt="Profile views counter" />
</p>

<p align="center">
  <a href="https://github.com/miguel50flowers">
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&size=20&pause=1200&color=8B5CF6&center=true&vCenter=true&width=640&lines=Full-Stack+Engineer+%E2%80%A2+10%2B+years+shipping+production+software;Ingeniero+Full-Stack+%E2%80%A2+10%2B+a%C3%B1os+enviando+software+a+producci%C3%B3n;Shopify+%C2%B7+TypeScript+%C2%B7+Bun+%C2%B7+Next.js+%C2%B7+Python+%C2%B7+AWS;Building+image+pipelines%2C+admin+APIs+y+macOS+tooling" alt="Typing intro: roles and stack" />
  </a>
</p>

---

## About me / Sobre mí

<table>
<tr>
<td width="50%" valign="top">

### EN

I'm Miguel — a full-stack engineer based in **Quito, Ecuador**, with **10+ years** shipping production software across the e-commerce, image-pipeline and macOS-tooling spaces.

Day-to-day I build **internal admin APIs and image sync pipelines** for a Shopify-based e-commerce: dockerized services, ETLs, and the kind of boring-but-load-bearing infrastructure that keeps thousands of SKUs in sync.

I also publish small open-source utilities for the problems I personally hit — see the *Featured* section below.

I care about **well-typed, well-logged, well-tested software that ships**.

</td>
<td width="50%" valign="top">

### ES

Soy Miguel — ingeniero full-stack en **Quito, Ecuador**, con **10+ años** entregando software a producción en e-commerce, pipelines de imagen y herramientas para macOS.

Día a día construyo **APIs admin internas y pipelines de sincronización de imágenes** para un e-commerce sobre Shopify: servicios dockerizados, ETLs e infraestructura aburrida-pero-crítica que mantiene miles de SKUs sincronizados.

También publico pequeñas utilidades open-source para los problemas que yo mismo encuentro — ve la sección *Featured* abajo.

Creo en **software bien tipado, bien logueado, bien probado, que se entrega**.

</td>
</tr>
</table>

---

## Currently working on / En lo que trabajo

- **Image sync pipelines** — bulk image processing & CDN sync for an e-commerce on Shopify (Bun · Next.js middleware · Docker).
- **Internal admin API** — backend para operaciones internas del mismo e-commerce (Bun · TypeScript · Prisma · Redis · Docker).
- **`fe-image-transform-lite`** — open-source macOS app for batch image transforms (Python · Pillow · pywebview).

---

## Tech stack

<p align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=ts,react,nextjs,nodejs,bun,python,swift,docker,aws,postgres,redis,git&perline=12" alt="Primary tech stack: TypeScript, React, Next.js, Node.js, Bun, Python, Swift, Docker, AWS, PostgreSQL, Redis, Git" />
  </a>
</p>

<p align="center"><sub>

Also working with: Shopify (GraphQL/REST/Liquid) · Remix · Hono · Express · AWS Lambda · MySQL · SQLite · GitHub Actions · Pillow · pywebview

</sub></p>

---

## GitHub stats

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://github-readme-stats.vercel.app/api?username=miguel50flowers&show_icons=true&hide_border=true&theme=tokyonight&rank_icon=github" />
    <source media="(prefers-color-scheme: light)" srcset="https://github-readme-stats.vercel.app/api?username=miguel50flowers&show_icons=true&hide_border=true&rank_icon=github" />
    <img src="https://github-readme-stats.vercel.app/api?username=miguel50flowers&show_icons=true&hide_border=true&theme=tokyonight&rank_icon=github" alt="GitHub stats card" height="170" />
  </picture>
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://github-readme-stats.vercel.app/api/top-langs/?username=miguel50flowers&layout=compact&hide_border=true&theme=tokyonight&langs_count=8&hide=html,css,scss" />
    <source media="(prefers-color-scheme: light)" srcset="https://github-readme-stats.vercel.app/api/top-langs/?username=miguel50flowers&layout=compact&hide_border=true&langs_count=8&hide=html,css,scss" />
    <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=miguel50flowers&layout=compact&hide_border=true&theme=tokyonight&langs_count=8&hide=html,css,scss" alt="Top languages card" height="170" />
  </picture>
</p>

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://streak-stats.demolab.com?user=miguel50flowers&hide_border=true&theme=tokyonight" />
    <source media="(prefers-color-scheme: light)" srcset="https://streak-stats.demolab.com?user=miguel50flowers&hide_border=true" />
    <img src="https://streak-stats.demolab.com?user=miguel50flowers&hide_border=true&theme=tokyonight" alt="Contribution streak card" />
  </picture>
</p>

---

## Activity insights / Mi actividad

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=miguel50flowers&theme=tokyonight" alt="Repositories grouped by primary language" height="180" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/productive-time?username=miguel50flowers&theme=tokyonight&utcOffset=-5" alt="Productive coding hours by time of day" height="180" />
</p>

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=miguel50flowers&theme=tokyo-night&hide_border=true&area=true&radius=8&custom_title=Contribution+activity+last+year" alt="Contribution activity over the last year (line graph)" />
</p>

---

## Featured projects / Proyectos destacados

<table>
<tr>
<td width="50%" valign="top">

### fe-image-transform-lite
[![fe-image-transform-lite repo card](https://github-readme-stats.vercel.app/api/pin/?username=miguel50flowers&repo=fe-image-transform-lite&hide_border=true&theme=tokyonight)](https://github.com/miguel50flowers/fe-image-transform-lite)

Batch image transform desktop app for **macOS** — flip, rotate, crop, resize, brightness/contrast/sharpness — drag-and-drop, presets, WebP/JPEG/PNG/TIFF.
**Python 3.11 · Pillow · pywebview**

</td>
<td width="50%" valign="top">

### openvpn-mac-fix
[![openvpn-mac-fix repo card](https://github-readme-stats.vercel.app/api/pin/?username=miguel50flowers&repo=openvpn-mac-fix&hide_border=true&theme=tokyonight)](https://github.com/miguel50flowers/openvpn-mac-fix)

Auto-restores macOS internet after disconnecting OpenVPN — a tiny Swift menu-bar helper. No more manual Wi-Fi toggle dance.
**Swift · macOS**

</td>
</tr>
</table>

> Most of my day-to-day work lives in private repos (Shopify integrations, ETLs, internal admin). Happy to walk through architecture and code on request.

---

## Contribution graph

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/miguel50flowers/miguel50flowers/output/github-snake-dark.svg" />
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/miguel50flowers/miguel50flowers/output/github-snake.svg" />
    <img src="https://raw.githubusercontent.com/miguel50flowers/miguel50flowers/output/github-snake.svg" alt="Snake-style animation eating GitHub contributions" />
  </picture>
</p>

---

## Reach me / Contacto

<p align="center">
  <a href="https://www.maecly.com/"><img src="https://img.shields.io/badge/Portfolio-maecly.com-8B5CF6?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Portfolio: maecly.com" /></a>
  <a href="https://www.linkedin.com/in/miguel-angel-esparza-calero-88ba49142/"><img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn profile" /></a>
  <a href="https://x.com/MigelAngel_ec"><img src="https://img.shields.io/badge/Twitter-%40MigelAngel__ec-000000?style=for-the-badge&logo=x&logoColor=white" alt="Twitter / X profile" /></a>
  <a href="mailto:migueltuxd@gmail.com"><img src="https://img.shields.io/badge/Email-migueltuxd%40gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email contact" /></a>
</p>

<p align="center"><sub>More links (Instagram, YouTube, Facebook, GitHub <code>@MigelAngelEC</code>) on <a href="https://www.maecly.com/">maecly.com</a>.</sub></p>

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12,2,20,30&height=80&section=footer" alt="Decorative footer wave" />
